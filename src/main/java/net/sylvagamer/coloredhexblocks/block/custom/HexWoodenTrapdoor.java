...
public class HexWoodenTrapdoor extends TrapdoorBlock implements BlockEntityProvider {
    public HexWoodenTrapdoor(BlockSetType type, Settings settings) {
        super(type, settings);
    }
    ...
    public @Nullable BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new HexWoodenTrapdoorEntity(pos, state);
    }

    public ItemStack pickBlock(BlockView world, BlockPos pos, ItemStack stack, boolean includeData){
        HexWoodenTrapdoorEntity blockEntity = ModBlockEntities.HEX_WOODEN_TRAPDOOR_ENTITY.get(world,pos);
        int color = HexWoodenTrapdoorEntity.DEFAULT_COLOR;
        if(blockEntity != null){
    ...
    public static class HexWoodenTrapdoorEntity extends BlockEntity {

        public static final int DEFAULT_COLOR = 16777215;
        public int color = DEFAULT_COLOR;

        public HexWoodenTrapdoorEntity(BlockPos pos, BlockState state) {
            super(ModBlockEntities.HEX_WOODEN_TRAPDOOR_ENTITY,pos, state);
        }
        ...
        public static int getColor(BlockRenderView world, BlockPos pos) {
            if(world==null){
                return HexWoodenTrapdoorEntity.DEFAULT_COLOR;
            }
            BlockEntity blockEntity = world.getBlockEntity(pos);
            if(blockEntity instanceof HexWoodenTrapdoorEntity hexBlockEntity){
