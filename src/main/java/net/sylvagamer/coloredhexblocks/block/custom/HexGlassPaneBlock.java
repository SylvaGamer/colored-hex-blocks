...
public class HexGlassPaneBlock extends StainedGlassPaneBlock implements BlockEntityProvider {
    ...
    public @Nullable BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new HexGlassPaneEntity(pos, state);
    }

    public ItemStack pickBlock(BlockView world, BlockPos pos, ItemStack stack, boolean includeData){
        HexGlassPaneEntity blockEntity = ModBlockEntities.HEX_GLASS_PANE_ENTITY.get(world,pos);
    ...
    public static class HexGlassPaneEntity extends BlockEntity {

        public static final int DEFAULT_COLOR = 16777215;
        public int color = DEFAULT_COLOR;

        public HexGlassPaneEntity(BlockPos pos, BlockState state) {
            super(ModBlockEntities.HEX_GLASS_PANE_ENTITY,pos, state);
        }
        ...
        public static int getColor(BlockRenderView world, BlockPos pos) {
            if(world==null){
                return HexGlassPaneEntity.DEFAULT_COLOR;
            }
            BlockEntity blockEntity = world.getBlockEntity(pos);
            if(blockEntity instanceof HexGlassPaneEntity hexBlockEntity){
