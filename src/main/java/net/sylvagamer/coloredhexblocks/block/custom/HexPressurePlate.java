...
public class HexPressurePlate extends PressurePlateBlock implements BlockEntityProvider {

    public HexPressurePlate(BlockSetType type, Settings settings) {
        super(type, settings);
    }
    ...
    public @Nullable BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new HexPressurePlateEntity(pos, state);
    }

    public ItemStack pickBlock(BlockView world, BlockPos pos, ItemStack stack, boolean includeData){
        HexPressurePlateEntity blockEntity = ModBlockEntities.HEX_PRESSURE_PLATE_ENTITY.get(world,pos);
        int color = HexPressurePlateEntity.DEFAULT_COLOR;
        if(blockEntity != null){
    ...
    public static class HexPressurePlateEntity extends BlockEntity {

        public static final int DEFAULT_COLOR = 16777215;
        public int color = DEFAULT_COLOR;

        public HexPressurePlateEntity(BlockPos pos, BlockState state) {
            super(ModBlockEntities.HEX_PRESSURE_PLATE_ENTITY,pos, state);
        }
        ...
        public static int getColor(BlockRenderView world, BlockPos pos) {
            if(world==null){
                return HexPressurePlateEntity.DEFAULT_COLOR;
            }
            BlockEntity blockEntity = world.getBlockEntity(pos);
            if(blockEntity instanceof HexPressurePlateEntity hexBlockEntity){
