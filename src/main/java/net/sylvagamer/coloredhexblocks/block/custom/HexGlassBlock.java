...
public class HexGlassBlock extends TransparentBlock implements BlockEntityProvider {
    ...
    public @Nullable BlockEntity createBlockEntity(BlockPos pos, BlockState state) {
        return new HexGlassBlockEntity(pos, state);
    }
    public ItemStack pickBlock(BlockView world, BlockPos pos, ItemStack stack, boolean includeData){
        HexGlassBlockEntity blockEntity = ModBlockEntities.HEX_GLASS_BLOCK_ENTITY.get(world,pos);
        int color = HexBlock.HexBlockEntity.DEFAULT_COLOR;
        if(blockEntity != null){
    ...
    public static class HexGlassBlockEntity extends BlockEntity {

        public static final int DEFAULT_COLOR = 16777215;
        public int color = DEFAULT_COLOR;

        public HexGlassBlockEntity(BlockPos pos, BlockState state) {
            super(ModBlockEntities.HEX_GLASS_BLOCK_ENTITY,pos, state);
        }
        ...
        public static int getColor(BlockRenderView world, BlockPos pos) {
            if(world==null){
                return HexGlassBlockEntity.DEFAULT_COLOR;
            }
            BlockEntity blockEntity = world.getBlockEntity(pos);
            if(blockEntity instanceof HexGlassBlockEntity hexBlockEntity){
