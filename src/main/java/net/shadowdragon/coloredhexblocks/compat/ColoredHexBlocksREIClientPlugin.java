...
public class ColoredHexBlocksREIClientPlugin implements REIClientPlugin {
    ...
    public void registerCategories(CategoryRegistry registry) {
        registry.add(new DyingStationCategory());

        registry.addWorkstations(DyingStationCategory.DYING_STATION, EntryStacks.of(ModHexBlocks.DYING_STATION));
    }

    @Override
    public void registerDisplays(DisplayRegistry registry) {
        registry.registerRecipeFiller(HexBlockRecipe.class, HexBlockRecipe.Type.INSTANCE,
                DyingStationDisplay::new);
    }

    @Override
    public void registerScreens(ScreenRegistry registry) {
        registry.registerClickArea(screen -> new Rectangle(((screen.width - 176)/2)+78,
                        ((screen.height - 166)/2)+30,20,30), DyingStationScreen.class,
                DyingStationCategory.DYING_STATION);
    }
}
